...
def split_text_by_sentence(text, max_words=200):
    """
    将文本按句子结尾分割，每段不超过max_words词，尽量在句子结尾断开。
    返回分段列表。
    """
    # 先按句子分割
...
def process_video_file(video_file):
    ...
    with open(en_txt_path, 'r', encoding='utf-8') as f:
        text = f.read()
    # 分段翻译
    segments = split_text_by_sentence(text, max_words=100)
    translated_segments = []
    for idx, seg in enumerate(segments):
        print(f"  正在翻译第{idx+1}/{len(segments)}段（约{len(seg.split())}词）...")
        zh = translate_text(seg)
        print("    段翻译前十词：", ' '.join(zh.split()[:10]))
        translated_segments.append(zh)
        time.sleep(5)
